...
class KGCN(BaseModel):
    ...
    def get_term_microbe_embedding(self, term):
        ...
        :param term: input_microbe [batch_size,1]           这个方法用于获取微生物的预训练嵌入表示。
    ...
    def get_term_disease_embedding(self, term):
        ...
        :param term: input_disease, shape [batch_size,1]  这个方法用于获取疾病的预训练嵌入表示。
        :return: pre_embedding_tensor,shape [batch_size,1,pre_embedding_dim]
        '''
    ...
    def get_receptive_field(self, entity):
        ...
        :return: a list of tensor: [[batch_size, 1], [batch_size, neighbor_sample_size],
             ...
             计算实体的接收字段。该方法根据输入实体，利用邻接矩阵找到其邻居实体和关联关系，为后续的聚合操作提供数据。
        """
    ...
    def get_neighbor_info(self, drug, rel, ent):
        ...
        根据当前实体的嵌入、邻居实体的嵌入和邻居关系的嵌入，计算邻居信息的加权嵌入表示
        """
    ...
    def add_metrics(self, x_train, y_train):                  # 添加自定义的评估指标到训练过程中，以便于监控。
        self.callbacks.append(KGCNMetric(x_train, y_train,self.config.aggregator_type, self.config.dataset, self.config.K_Fold))


    # 训练模型。该方法接受训练数据x_train和y_train，并执行模型的训练过程，包括初始化回调函数、设置批量大小、训练周期等。
    ...
    def predict(self, x):         # 对给定的输入x进行预测，返回预测结果。
        return self.model.predict(x).flatten()

    # 估模型在给定数据x和标签y上的性能，包括计算AUC、准确率、F1得分和AUPR。
    def score(self, x, y, threshold=0.5):
        ...
        return auc, acc, f1, aupr, y_true, y_pred

    # 获取模型在给定数据x上的真实标签y和预测结果，用于进一步的评估分析。
    # 这个类通过构建和训练知识图谱卷积网络，实现了对微生物和疾病之间关系的预测。
