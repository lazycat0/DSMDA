...
matplotlib.use('TkAgg')    # 解决module 'backend_interagg' has no attribute 'FigureCanvas'
...
def S_fun1(DDsim, T0, T1):
    ...
    if len(T0) != 0 and len(T1) != 0:
        for ti in T0:
            ...
            T0_T1.append(max(m_ax))       # 计算第一组微生物相关疾病中的一个与第二组微生物相关疾病的最大相似度
...
# 计算Fs
def FS_fun1(T0_T1, T1_T0, T0, T1):      #通过疾病的相似度计算微生物的相似度
    a = len(T1)
    b = len(T0)
    S1 = sum(T0_T1)
    S2 = sum(T1_T0)
...
# Gaussian interaction profile kernel similarity  高斯交互剖面核相似度
def r_func(MD):
    m = MD.shape[0]
    n = MD.shape[1]
...
MD = pd.read_csv("../Data/MD_A.csv",index_col=0)         #微生物-疾病
MD
...
DS = pd.read_csv("../Data/DS.csv",index_col=0)          #疾病-疾病
DS
...
rm, rt = r_func(MD)
GaM, GaD = Gau_sim(MD, rm, rt)          #计算高斯交互剖面核相似度
...
cos_MS, cos_DS=cos_sim(MD_c)    # 计算微生物和疾病的余弦相似度
...
sig_MS,sig_DS = sig_kr(MD_c)      #微生物和疾病的Sigmoid核函数相似性
...
for train_index, test_index in kf.split(SAMPLE_sub):
    ...
    metrics_summary['f1_scores'].append(f1_score(test_labels, test_predict))
    metrics_summary['accuracies'].append(accuracy_score(test_labels, test_predict))
    metrics_summary['recalls'].append(recall_score(test_labels, test_predict))
    metrics_summary['precisions'].append(precision_score(test_labels, test_predict))
    tn, fp, fn, tp = confusion_matrix(test_labels, test_predict).ravel()
    specificity = tn / (tn + fp)
    metrics_summary['specificities'].append(specificity)

    fpr_temp, tpr_temp, _ = roc_curve(test_labels, test_predict)
    roc_auc = auc(fpr_temp, tpr_temp)
    sk_fpr.append(fpr_temp)
    sk_tprs.append(tpr_temp)
    sk_aucs.append(roc_auc)

    precision, recall, _ = precision_recall_curve(test_labels, test_predict)
    pr_auc = auc(recall, precision)


    # 计算Precision-Recall曲线和AUPR
    precision_temp, recall_temp, _ = precision_recall_curve(test_labels, test_predict)
    ...
    with open(fold_metrics_file, mode='a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(
            [j , roc_auc, pr_auc, f1_score(test_labels, test_predict), accuracy_score(test_labels, test_predict)])
